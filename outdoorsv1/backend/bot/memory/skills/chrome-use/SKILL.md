...
## Google OAuth Consent Screen — CANNOT BE AUTOMATED
- Google's OAuth consent/account-chooser pages **block all automated click methods**:
  - JS `.click()`, `dispatchEvent(MouseEvent)`, `Input.dispatchMouseEvent`, `Input.dispatchKeyEvent` — all silently fail
  - The page stays on the consent screen; no navigation occurs
- **DO NOT waste turns retrying** different click strategies — this is a hard block by Google
- **Workarounds**: (1) user clicks manually, (2) navigate directly to the destination URL if already authenticated, (3) use API tokens instead of browser-based OAuth
