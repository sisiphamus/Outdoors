...
async function processIncomingMessage(handle, text, rawMsg) {
    ...
    try {
      ...
      if (onboardingResult.done) {
        ...
        const commandsText = `you can run multiple conversations at once:\n\n` +
          ...
          `- pause one: *1 pause*\n` +
          `- done with one: *1 new*`;
        await sendWithRetry(handle, formatOutdoorsResponse(commandsText));
      }
    } catch (err) {
  ...
  if (refState) {
    if (refState.stage === 'name') {
      await sendWithRetry(handle, formatOutdoorsResponse('Looking up contacts...'));
    }
    const { executeCodexPrompt } = await import('./codex-bridge.js');
    const replyFn = async (m) => await sendWithRetry(handle, formatOutdoorsResponse(m));
    ...
    if (result?.handled) {
      await sendWithRetry(handle, formatOutdoorsResponse(result.reply));
      return;
    }
  }
  ...
  if (/^(invite|refer)\b/i.test(text)) {
    ...
    await sendWithRetry(handle, formatOutdoorsResponse(refResult.prompt));
    return;
  }
  ...
  if (!hasQuota()) {
    ...
    await sendWithRetry(handle, formatOutdoorsResponse(reply));
    return;
  }
  ...
  try {
    ...
    if (!handlerErr.stopped) {
      ...
      try {
        await sendWithRetry(handle, formatOutdoorsResponse('Something went wrong processing your message. Please try again.'));
      } catch {}
    }
    return;
  }
  ...
  if (!result.response) {
    try {
      await sendWithRetry(handle, formatOutdoorsResponse('I processed your message but the response was empty. Try again?'));
    } catch {}
  ...
  await withSendLock(handle, async () => {
    ...
    try {
      ...
      // Send text with Outdoors formatting
      if (cleanText) {
      ...
      if (!cleanText && images.length === 0) {
        ...
        await sendWithRetry(handle, formatOutdoorsResponse('I processed your message but had nothing to say. Try rephrasing?'));
      }
 ...
 * Used by trigger/automation schedulers — equivalent to sendToOutdoorsGroup.
 */
