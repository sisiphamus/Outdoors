...
import { ensureBrowserReady } from '../../chieftonv1/backend/src/browser-health.js';
    ...
    try {
      ...
      if (modelErr.authExpired || modelErr.authMissing) {
        agg.phase('auth', modelErr.authExpired
          ? 'Codex login expired — please re-authenticate in the Chiefton dashboard.'
          : 'Codex login not found — please authenticate in the Chiefton dashboard.');
        return {
          status: 'auth_expired',
          response: modelErr.authExpired
            ? 'Your AI session has expired. Please open Chiefton on your computer and click Re-authenticate in the dashboard, or run "codex auth --login" in a terminal.'
            : 'Codex is not authenticated. Please open the Chiefton app and complete the setup wizard.',
          sessionId: null,
          fullEvents: [],
        };
      }
