# chieftonv4 — Multi-Model Orchestration Pipeline

## What is this?

chieftonv4 replaces chieftonv1's `claude-bridge.js` — the layer that takes a user message and turns it into a Claude response. Instead of a single Claude call, chieftonv4 runs a 4-model pipeline that analyzes the problem, gathers the right knowledge, fills gaps, and executes with full context.

chieftonv1's server, messaging channels (WhatsApp, web Socket.IO), and conversation management are unchanged. chieftonv4 only replaces the "brain" — how messages get processed.

Built fresh from chieftonv1's foundation. No code from chieftonv2 or chieftonv3. chieftonv4 has zero source imports from chieftonv1 — it has its own copy of config.js and only shares the runtime config.json and memory directory as data.
...
User Message
    |
    |— simple? → Fast-path: single Claude call (same as chieftonv1)
    |
...
chieftonv4/
  ...
  config.js                       — Config loader (reads chieftonv1/backend/config.json)
  claude-bridge.js                — Drop-in replacement for chieftonv1's claude-bridge.js
  runtime-health.js               — Drop-in replacement for chieftonv1's runtime-health.js
  pipeline/
...
chieftonv4 manages 4 categories of memory, all stored in `chieftonv1/backend/bot/memory/`:
...
## How to enable chieftonv4

chieftonv4 does NOT modify chieftonv1 files. Instead, it provides drop-in replacements:

1. Copy `chieftonv4/claude-bridge.js` → `chieftonv1/backend/src/claude-bridge.js`
2. Copy `chieftonv4/runtime-health.js` → `chieftonv1/backend/src/runtime-health.js`
3. Set env var: `CHIEFTON_V4_ENABLED=true`
4. Start normally: `npm run dev`

To revert, restore the original files or set `CHIEFTON_V4_ENABLED=false`.

## Fast-path vs pipeline

- **Fast-path**: Messages under 200 chars matching simple patterns (questions, greetings) skip the pipeline and get a single Claude call. Same speed as chieftonv1.
