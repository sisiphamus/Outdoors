...
import * as clarifications from '../../../chieftonv4/memory/clarification-manager.js';
...
describe('clarification-manager', () => {
  it('persists and clears pending clarification state', () => {
    ...
    clarifications.appendAnswer('test:chat:1', 'It is for Chiefton');
    ...
    assert.match(augmented, /It is for Chiefton/);
...
describe('model-runner safety guards', () => {
  it('uses platform-aware shell setting', () => {
    const src = readFileSync(join(process.cwd(), '..', '..', 'chieftonv4', 'pipeline', 'model-runner.js'), 'utf-8');
