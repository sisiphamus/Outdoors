...
const DOWNLOAD_URL_WINDOWS = 'https://github.com/sisiphamus/Outdoors/releases/latest/download/Chiefton-Setup.exe';
const DOWNLOAD_URL_MAC = 'https://github.com/sisiphamus/Outdoors/releases/latest/download/Chiefton.dmg';
...
export async function processReferralReply(jid, text, executePrompt, replyFn, killProcessFn) {
  ...
  if (state.stage === 'message') {
    ...
    const preview = `Here's what will be sent to *${state.friendName} ${state.friendLast}* (${state.email}):\n\n`
      + `Subject: You're Invited to Chiefton\n\n`
      + `Hey ${state.friendName},\n\n`
      + `You're Invited! ${state.senderName} has been using Chiefton and you get to be one of the first users.\n\n`
      + `$100 in free usage thanks to OpenAI <3${personalNote}\n\n`
      + `Chiefton is a personal AI assistant that works through WhatsApp. It can send emails, manage your calendar, build websites, do research, and way more.\n\n`
  ...
  if (state.stage === 'confirm') {
    ...
    const codeBlock = inviteCode
      ? `\n\nYour invite code: ${inviteCode}\n\nDownload Chiefton:\n• Windows: ${DOWNLOAD_URL_WINDOWS}\n• Mac: ${DOWNLOAD_URL_MAC}\n\nAfter installing, open Chiefton and enter your invite code when prompted.`
      : `\n\n(Invite code generation failed — ask ${senderName} to send you a code directly.)`;
    const emailBody = `Hey ${friendName},\n\nYou're Invited! ${senderName} has been using Chiefton and you get to be one of the first users.\n\n$100 in free usage thanks to OpenAI <3${personalNote}\n\nChiefton is a personal AI assistant that works through WhatsApp. It can send emails, manage your calendar, build websites, do research, and way more.${codeBlock}`;
    sendEmail({
        ...
        if (bounced) {
          ...
          if (foundEmail && !isAlreadyReferred(foundEmail) && !isSelfEmail(foundEmail)) {
            ...
            sendEmail({
              ...
              subject: "You're Invited to Chiefton", body: emailBody,
            }).catch(() => {});
