...
async function sendOnboardingWelcome(sock, groupJid) {
  try {
    ...
    await sock.sendMessage(groupJid, { text: 'Chiefton is ready! Send a message here to get started.' }).catch(() => {});
  }
}
...
async function createChieftonGroup(sock, emitLog) {
  try {
    ...
    saveConfig(config);
    console.log(`[WhatsApp] Created Chiefton group: ${groupJid}`);
    emitLog('group_created', { groupJid, message: 'Chiefton group created — open it in WhatsApp to start chatting' });

    await sock.groupUpdateDescription(groupJid, 'Send messages here to chat with Chiefton.').catch(() => {});
    ...
    console.log('[WhatsApp] Failed to create Chiefton group:', err.message);
...
async function maybePrimeOwnDeviceCache() {
  ...
  if (!sock?.user?.lid || !config.chieftonGroupJid) return;
  const meta = await ensureGroupMeta(config.chieftonGroupJid);
...
async function startWhatsApp() {
  ...
  sock.ev.on('connection.update', (update) => {
    ...
    if (connection === 'open') {
      ...
      if (!config.chieftonGroupJid) {
        // Search for the most recently active Chiefton group, or create a new one
        (async () => {
          try {
            const groups = await sock.groupFetchAllParticipating();
            const chieftonGroups = Object.values(groups)
              .filter(g => g.subject && g.subject.toLowerCase().includes('chiefton'))
              .sort((a, b) => {
            ...
            if (chieftonGroups.length > 0) {
              const chieftonGroup = chieftonGroups[0]; // most recently active
              config.chieftonGroupJid = chieftonGroup.id;
              saveConfig(config);
              console.log(`[WhatsApp] Found ${chieftonGroups.length} Chiefton group(s), using most recent: ${chieftonGroup.id} ("${chieftonGroup.subject}")`);
              emitLog('group_found', { groupJid: chieftonGroup.id, subject: chieftonGroup.subject });
            } else {
              createChieftonGroup(sock, emitLog);
            }
          } catch (err) {
        ...
        console.log(`[WhatsApp] Using existing Chiefton group: ${config.chieftonGroupJid}`);
        (async () => {
          try {
            if (isOnboardingNeeded()) {
              await sendOnboardingWelcome(sock, config.chieftonGroupJid);
            }
          } catch (err) {
  ...
  sock.ev.on('messages.upsert', (upsert) => {
    ...
    for (const msg of messages) {
      ...
      // Only accept messages from the Chiefton group — ignore DMs and other groups
      if (config.chieftonGroupJid) {
        if (remoteJid !== config.chieftonGroupJid) {
          continue; // Not the Chiefton group — skip silently
        }
      } else {
      ...
      (async () => {
        ...
        try {
              ...
              try {
                ...
                if (onboardingResult.done) {
                  ...
                  const commandsText = `you can run multiple conversations at once:\n\n` +
                    ...
                    `- pause one: *1 pause*\n` +
                    `- done with one: *1 new*`;
                  const commandsMsg = formatChieftonResponse(commandsText);
          ...
          if (refState) {
            ...
            if (result?.handled) {
              const sent = await sendWithRetry(jid, { text: formatChieftonResponse(result.reply) });
          ...
          if (/^(invite|refer)\b/i.test(msgText)) {
            ...
            const sent = await sendWithRetry(jid, { text: formatChieftonResponse(refResult.prompt) });
            if (sent?.key?.id) { addBotSentId(sent.key.id); storeMessage(sent.key.id, sent.message); }
            return;
          }
          ...
          if (!hasQuota()) {
            ...
            const sent = await sendWithRetry(jid, { text: formatChieftonResponse(reply) });
            if (sent?.key?.id) { addBotSentId(sent.key.id); storeMessage(sent.key.id, sent.message); }
            return;
          }
          ...
          if (!handlerErr.stopped) {
            ...
            try {
              const errorMsg = formatChieftonResponse(`Something went wrong processing your message. Please try again.`);
        ...
        if (!result) {
          ...
          try {
            const fallback = formatChieftonResponse('I processed your message but the response was empty. Try again?');
        ...
        if (result && result.response) {
          ...
          await withSendLock(result.jid, async () => {
            ...
            try {
              ...
              // Send text in chunks (~4000 chars each) with Chiefton formatting
              if (cleanText) {
              ...
              if (!cleanText && images.length === 0) {
                ...
                const fallback = formatChieftonResponse(`I processed your message but had nothing to say. Try rephrasing?`);
...
/**
 * Send a text message to the Chiefton WhatsApp group.
...
async function sendToChieftonGroup(text) {
  const groupJid = config.chieftonGroupJid;
  if (!groupJid) {
    console.log('[WhatsApp] Cannot send — no Chiefton group configured');
    return null;
  }
  if (!sock || connectionStatus !== 'connected') {
...
export { startWhatsApp, setSocketIO, getStatus, getLastQR, reconnectWhatsApp, sendToChieftonGroup };
