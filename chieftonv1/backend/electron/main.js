/**
 * Chiefton — Electron Main Process
 *
 * Thin shell that:
...
const BACKEND_DIR = IS_DEV
  ? path.join(__dirname, '..')
  : path.join(WORKSPACE, 'chieftonv1', 'backend');
const SETUP_DIR = IS_DEV
  ? path.join(__dirname, '..', 'src', 'setup')
  : path.join(WORKSPACE, 'chieftonv1', 'backend', 'src', 'setup');
...
function ensureWorkspace() {
  ...
  const backendIndex = path.join(WORKSPACE, 'chieftonv1', 'backend', 'src', 'index.js');
  const v4Index = path.join(WORKSPACE, 'chieftonv4', 'index.js');
  ...
  const destBackend = path.join(WORKSPACE, 'chieftonv1', 'backend');
  ...
  if (!isFirstRun) {
    ...
    try {
      ...
      const bundledV4 = path.join(process.resourcesPath, 'chieftonv4');
      if (fs.existsSync(bundledV4)) copyDirSync(bundledV4, path.join(WORKSPACE, 'chieftonv4'));
    } catch (err) {
...
function setupIPC() {
  ...
  function runAsAdmin(shellCmd, description) {
    return new Promise((resolve) => {
      ...
      const script = `do shell script "${escaped}" with prompt "Chiefton needs your password to ${description || 'install dependencies'}" with administrator privileges`;
    ...
    return new Promise((resolve) => {
      ...
      const tick = () => {
        ...
        if (Date.now() > deadline) {
          ...
          resolve({ ok: false, output: 'CLT install timed out — please finish it from the System dialog and restart Chiefton' });
          return;
        }
        ticks++;
        if (ticks % 6 === 0) {
  ...
  ipcMain.handle('install-system-deps', async () => {
    ...
    try { fs.writeFileSync(INSTALL_LOG_PATH, `=== Chiefton install log (${new Date().toISOString()}) ===\n`); } catch {}
  ...
  ipcMain.handle('install-node-deps', async () => {
    ...
    try {
      ...
      return { ok: false, output: 'npm_not_found', error: 'Node.js is not installed. Please install Node.js from https://nodejs.org and restart Chiefton.' };
    }
        ...
        for (const authPath of authPaths) {
          try {
            if (fs.existsSync(authPath)) {
              ...
              // Bring Chiefton window to front
              if (mainWindow && !mainWindow.isDestroyed()) {
...
`;
  ...
  ipcMain.handle('check-google-creds', () => {
    try {
      const credsPath = IS_DEV
        ...
        : path.join(WORKSPACE, 'chieftonv1', 'backend', 'oauth-creds.json');
        ...
        async function doMcpAuth() {
          try {
            ...
            await mcpPost({
              ...
              params: { protocolVersion: '2024-11-05', capabilities: {}, clientInfo: { name: 'chiefton-setup', version: '1.0' } },
            });
  ...
  ipcMain.handle('run-onboarding-scan', async (_event, services) => {
    try {
      ...
      const fullPrompt = `You are running an onboarding personalization scan for a new Chiefton user. Your goal is to use Google Workspace MCP tools (mcp__google_workspace__*) to learn about the user and fill out standardized knowledge files.
...
Start by reading the skill file, then scan each service systematically.`;
      ...
      // Ensure mcp-bot.json exists in chieftonv4 with google_workspace
      const v4Dir = IS_DEV
        ? path.join(__dirname, '..', '..', '..', 'chieftonv4')
        : path.join(WORKSPACE, 'chieftonv4');
      ...
      const promptFile = path.join(app.getPath('temp'), 'chiefton-onboarding-prompt.txt');
...
`;
  ...
  ipcMain.handle('save-full-config', async (_event, data) => {
    let cfg = {};
    try { if (fs.existsSync(CONFIG_PATH)) cfg = JSON.parse(fs.readFileSync(CONFIG_PATH, 'utf-8')); } catch {}
    const ALLOWED_CONFIG_KEYS = ['rateLimitPerMinute', 'maxResponseLength', 'messageTimeout', 'prefix', 'allowedNumbers', 'chieftonGroupJid', 'preferredBrowser'];
...
function writeMcpConfig(browserKey, browser) {
  ...
  // Also write mcp-bot.json in chieftonv4/ — browser-only tools for the bot runtime.
...
function ensureBackendDeps() {
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      return reject(new Error('Node.js / npm not found in PATH. Install Node.js from https://nodejs.org and relaunch Chiefton.'));
    }
...
function startBackend() {
  ...
  return new Promise((resolve) => {
    ...
    const onOutput = (data) => {
      ...
      if (!started && (text.includes('API: http://') || text.includes('Chiefton Bot'))) {
        started = true;
        backendStarting = false;
        resolve({ ok: true });
      }
