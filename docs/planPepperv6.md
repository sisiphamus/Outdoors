...
| PepperV6 Source | Outdoors Destination | Notes |
|---|---|---|
| `pepperv4/ml/infer.py` | `outdoorsv4/ml/infer.py` | **Full replace** — completely rewritten for dual-axis classifier + tiered retrieval |
| `pepperv4/ml/train.py` | `outdoorsv4/ml/train.py` | **Full replace** — major rewrite (+519/-242 lines) |
| `pepperv4/ml/models/phase_a_v2.pkl` | `outdoorsv4/ml/models/phase_a_v2.pkl` | Binary model file. Keep old `phase_a.pkl` as backup |
| `pepperv4/ml/data/real_examples.json` | `outdoorsv4/ml/data/real_examples.json` | New file — 56 labeled training examples |
| `pepperv4/ml/build_training_data.py` | `outdoorsv4/ml/build_training_data.py` | New utility |
| `pepperv4/ml/test_inference.py` | `outdoorsv4/ml/test_inference.py` | New utility |
| `pepperv1/backend/bot/memory/sites/google-docs.md` | `outdoorsv1/backend/bot/memory/sites/google-docs.md` | New site memory |
| `pepperv1/backend/bot/memory/sites/gradescope.md` | `outdoorsv1/backend/bot/memory/sites/gradescope.md` | New site memory |
| `pepperv1/backend/bot/memory/sites/io-rice.md` | `outdoorsv1/backend/bot/memory/sites/io-rice.md` | New site memory |
| `pepperv1/backend/bot/memory/sites/mcmurtry.md` | `outdoorsv1/backend/bot/memory/sites/mcmurtry.md` | New site memory |
| `pepperv1/backend/bot/memory/sites/rice-blogs.md` | `outdoorsv1/backend/bot/memory/sites/rice-blogs.md` | New site memory |
| `pepperv1/backend/bot/memory/sites/rice-edu.md` | `outdoorsv1/backend/bot/memory/sites/rice-edu.md` | New site memory |
| `pepperv1/backend/bot/memory/sites/slack.md` | `outdoorsv1/backend/bot/memory/sites/slack.md` | New site memory |
| `pepperv1/backend/bot/memory/sites/canvas.md` | `outdoorsv1/backend/bot/memory/sites/canvas.md` | Updated existing |
| `pepperv1/backend/bot/memory/sites/gmail.md` | `outdoorsv1/backend/bot/memory/sites/gmail.md` | Updated existing |
| `pepperv1/backend/bot/memory/skills/ocr-correction/` | `outdoorsv1/backend/bot/memory/skills/ocr-correction/` | New skill directory |
| `pepperv1/backend/bot/memory/skills/ocr-pipeline-development/` | `outdoorsv1/backend/bot/memory/skills/ocr-pipeline-development/` | New skill directory |
| `pepperv1/backend/bot/memory/skills/website-deployment/` | `outdoorsv1/backend/bot/memory/skills/website-deployment/` | New skill directory |

### Group 2: Merge Carefully (both sides have changes)

#### `orchestrator.js` — `pepperv4/pipeline/orchestrator.js` → `outdoorsv4/pipeline/orchestrator.js`
...
**Keep Outdoors-only changes (NOT in PepperV6):**
...
#### `ml-runner.js` — `pepperv4/pipeline/ml-runner.js` → `outdoorsv4/pipeline/ml-runner.js`
...
**Keep Outdoors-only changes:**
- Any `allowBrowser`, `maxTurns`, or `BASE_TOOLS`/`BROWSER_TOOLS` split changes

#### `model-d.js` — `pepperv4/pipeline/prompts/model-d.js` → `outdoorsv4/pipeline/prompts/model-d.js`
...
**Keep Outdoors-only changes** to this file (review diff before merging).

#### `learner.js` — `pepperv4/pipeline/prompts/learner.js` → `outdoorsv4/pipeline/prompts/learner.js`
...
#### `memory-manager.js` — `pepperv4/memory/memory-manager.js` → `outdoorsv4/memory/memory-manager.js`
...
| File | Why Skip |
|---|---|
| `pepperv4/config.js` | Changes `outputDirectory` path to `pepperv1/...` — Outdoors already has correct `outdoorsv1/...` equivalent |
| `pepperv1/backend/src/telegram/bot.js` | Telegram not used in Outdoors |
| `pepperv1/backend/bot/memory/skills/chrome-use/SKILL.md` | Already deleted in Outdoors (`git status` shows `D`) |
...
cp /tmp/pepperv6/pepperv4/ml/infer.py outdoorsv4/ml/infer.py
cp /tmp/pepperv6/pepperv4/ml/train.py outdoorsv4/ml/train.py
cp /tmp/pepperv6/pepperv4/ml/models/phase_a_v2.pkl outdoorsv4/ml/models/phase_a_v2.pkl
mkdir -p outdoorsv4/ml/data
cp /tmp/pepperv6/pepperv4/ml/data/real_examples.json outdoorsv4/ml/data/real_examples.json
cp /tmp/pepperv6/pepperv4/ml/build_training_data.py outdoorsv4/ml/build_training_data.py
cp /tmp/pepperv6/pepperv4/ml/test_inference.py outdoorsv4/ml/test_inference.py
...
for f in google-docs gradescope io-rice mcmurtry rice-blogs rice-edu slack canvas gmail; do
  cp /tmp/pepperv6/pepperv1/backend/bot/memory/sites/${f}.md \
     outdoorsv1/backend/bot/memory/sites/${f}.md
done
...
for s in ocr-correction ocr-pipeline-development website-deployment; do
  cp -r /tmp/pepperv6/pepperv1/backend/bot/memory/skills/${s} \
        outdoorsv1/backend/bot/memory/skills/${s}
done

# 3. Merge Group 2 files manually (see detailed instructions above)
# These need careful line-by-line merging to preserve Outdoors-only changes

# 4. Verify
python outdoorsv4/ml/train.py          # Should train with 88%+ accuracy
python outdoorsv4/ml/test_inference.py  # Should pass all tests
...
3. **Orchestrator merge is the hardest**: Both sides modified heavily. Work through the 9 PepperV6 changes + 4 Outdoors-only preservations carefully
